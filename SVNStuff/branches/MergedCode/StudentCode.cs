...
namespace PiEAPI
{
    public class StudentCode
    {
        ...
        private I2CMotorController i2c1;
        private I2CMotorController i2c2;
        ...
        private AnalogReflectanceSensor sensor1;
        private AnalogReflectanceSensor sensor2;
        private AnalogReflectanceSensor sensor3;
        private AnalogReflectanceSensor sensor4;
        private int rfR1;
        private int rfR2;
        private int rfL1;
        private int rfL2;
        ...
        public void UserControlledCode()
        {
           // Debug.Print("User Controlled");
            ...
            robot.radio.telemetry.analog[0] = (byte)(sensor1.Read()/4);
            robot.radio.telemetry.analog[1] = (byte)(sensor2.Read()/4);
            robot.radio.telemetry.analog[2] = (byte)(sensor3.Read()/4);
            robot.radio.telemetry.analog[3] = (byte)(sensor4.Read()/4);

            Debug.Print(""+(byte)(sensor1.Read()/4));
            Debug.Print("" + (byte)(sensor2.Read() / 4));
            Debug.Print("" + (byte)(sensor3.Read() / 4));
            Debug.Print("" + (byte)(sensor4.Read() / 4));

            robot.radio.InterfacePrint("sensor1: " + (byte)(sensor1.Read() / 4) + "\n" + 
                                       "sensor2: " + (byte)(sensor2.Read() / 4) + "\n" +
                                       "sensor3: " + (byte)(sensor3.Read() / 4) + "\n" +
                                       "sensor4: " + (byte)(sensor4.Read() / 4) + "\n");
           ...
           /**///rfR1 = rfR2;
            ...
            if (robot.UIDigitalVals[0])
            {
                ...
                if (i2c1.motorBrake < 10)
                {
                    //smcR.motorBrake <10
                    i2c1.motorBrake = i2c1.motorBrake + (float).1;
                    i2c2.motorBrake = i2c2.motorBrake + (float).1;//let's represent fractions with floats. always cast decimals to float
