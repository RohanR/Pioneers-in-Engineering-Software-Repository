...
namespace PiEAPI
{
    public class StudentCode
    {
        ...
        private OutputPort outp;

        private AnalogSensor sensor = new AnalogSensor(0);
        ///private OutputPort light = new OutputPort((Cpu.Pin)FEZ_Pin.Digital.IO62, false);
        
        ///Program stuff
        public Application program = new Application();
        ...
        {
            ///Debug.Print(refl.Read().ToString());
            if (sensor.Read() < .67 * 1023) {
                ///OutputPort light = new OutputPort((Cpu.Pin)FEZ_Pin.Digital.IO62, true);
            }

            Debug.Print("Student Code");
            ...
            if (refl.Read())
            {
                ...
                timer = 0;
                refl.ReflectanceSensor.Dispose();
                refl = new DigitalReflectanceSensor(0);
            }
             * 
            ...
            if ((timer % 10) == 0)
            {
                Port.ReservePin((Cpu.Pin)FEZ_Pin.Digital.IO45, true);
                Port.ReservePin((Cpu.Pin)FEZ_Pin.Digital.IO45, false);
                refl = new DigitalReflectanceSensor(1);
              refl.ReflectanceSensor.Dispose();
                refl = new DigitalReflectanceSensor(1);
            }
            */

            ///outp = new OutputPort((Cpu.Pin)FEZ_Pin.Digital.IO45, false);
            ///refl = new DigitalDistanceSensor(1);
           

           ///Bitmap LCD = new Bitmap(1000,
           ///                        1000);
