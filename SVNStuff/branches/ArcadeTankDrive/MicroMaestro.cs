...
namespace PiEAPI
{
    ...
    public class MicroMaestro : ActuatorController
    {
        ...
        private float[] speedsCopy;
        private float[] targetsCopy;
                ...
                for (int i = 0; i < 6; i++)
                {
                    if (targets[i] != -1)
                    {
                        ...
                        if (speeds[i] != speedsCopy[i])
                        {
                            ...
                            buffer[2] = (byte)0x07;
                            buffer[3] = (byte)i;
                            // 255 = max speed value maestro accepts, 1 = min value
                            int speed;
                            if (speeds[i] >= 100)
                                ...
                                speed = 1 + (int)((speeds[i]) * (255 - 1) / 100);
                            buffer[4] = (byte)(speed & 0x7F);
                            buffer[5] = (byte)((speed >> 7) & 0x7F);
                            port.Write(buffer, 0, 6);
                            speedsCopy[i] = speeds[i];
                        }

                        if (targets[i] != targetsCopy[i])
                        {
                            //update target
                            buffer[0] = (byte)0xAA;
                            buffer[1] = (byte)deviceNumber;
                            buffer[2] = (byte)0x04;
                            buffer[3] = (byte)i;
                            //13120 = max target value micro maestro accepts, 256 = min value
                            int target;
                            if(targets[i] >= maxRotation[i])
                                target = 13120;
                            else if(targets[i] <= minRotation[i])
                                target = 256;
                            else
                                target = 256 + (int)((targets[i] - minRotation[i]) * (13120 - 256) / (float)(maxRotation[i] - minRotation[i]));
