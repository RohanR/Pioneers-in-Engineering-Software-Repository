...
namespace PiEAPI
{
    ...
    public class Radio_Series2
    {
        ...
        public Radio_Series2(string portName)//Robot robo, string portName)
        {
            //robot = robo;
                ...
                if (port.BytesToRead > 0)
                {
                    ...
                    if (availableBytes > 0)
                    {
                        if (receiver.Fill(buffer, availableBytes))
                        {
                            ...
                            if (receiver.packet.data[0] == (byte)INTERFACEPACKET.IDENT)
                            {
                                ...
                                //robot.InterfacePacketReceived(data.analog, data.digital);
                                Debug.Print("********************RADIO lock**************");
                                if(true)//lock (robot.radioLock)
                                {
                                    for (int i = 0; i < UIAnalogVals.Length; i++)
                                    {
                                        UIAnalogVals[i] = data.analog[i];
                                    }
                                    for (int i = 0; i < UIDigitalVals.Length; i++)
                                    {
                                        UIDigitalVals[i] = data.digital[i];
                                    }
                                }
                            }
                            else if (receiver.packet.data[0] == (byte)FIELDPACKET.IDENT)
                            {
                                //Debug.Print("PACKET IS INTERFACE PACKET");
                                data.FillData(receiver.packet.data);
                                ...
                                if(true)//lock (robot.radioLock)
                                {
                                    for (int i = 0; i < FieldAnalogVals.Length; i++)
                                    {
                                        FieldAnalogVals[i] = data.analog[i];
                                    }
                                    for (int i = 0; i < FieldDigitalVals.Length; i++)
                                    {
                                        FieldDigitalVals[i] = data.digital[i];
                                    }
                                }
                            }
    ...
    public class InterfacePacket
    {
        ...
        public void FillData(byte[] bytes)
        {
            ...
            for (i++; i < analog.Length + digitalByteCount + 1; i++)
            {
                ...
                Debug.Print("analog length: " + analog.Length + " digitalByteCount: " + digitalByteCount + " i: " + i);
    ...
    public class XBeeReceiver
    {
        ...
        private Radio_Series2 parent;

        public XBeeReceiver(Radio_Series2 p)
        {
