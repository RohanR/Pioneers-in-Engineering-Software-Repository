...
namespace PiEAPI
{
    public class StudentCode
    {
        ...
        */
        ...
        // state-controlled autonomous code
        public void AutonomousCode()
        {
            // build state collection here
            // the array index of the state should be the same as its stateNumber!
            // the FSM starts at state 0.
            State goForward = new State(0, noOp, goFwd, stop);
            State goBackward = new State(1, noOp, goBck, stop);
            
            State[] autonStates = {goForward, goBackward};

            // instantiate FSM here
            FiniteStateMachine auton = new FiniteStateMachine(autonStates);

            while (/*robot.isAutonomous*/ true)
            {
                auton.update();
            }
        }
