...
namespace PiEAPI
{
    ...
    //interfacepacket is id 254 (255 is reserved by xbee)
    ...
    //pingpacket is ID 252, will be interfacepacket with different ident (received by robot)
    ...
    //pongpacket is ID 251, will be telemetrypacket with different ident (sent by robot)
     enum PONGPACKET
     {
      IDENT = 0xFB,
      ANALOG_BYTES = 7,
    ...
    public class Radio_Series1
    {
        ...
        public InterfacePacket ping;
        public TelemetryPacket pong;
        public long lastUpdate;
        public bool pinged = false; 
        //tells robot whether or not it needs to pong the computer back, true if robot has received ping and has not yet sent pong
        ...
        public Radio_Series1(Robot robo, string portName)
        {
            ...
            ping = new InterfacePacket((byte)PINGPACKET.IDENT, (int)PINGPACKET.ANALOG_BYTES, (int)PINGPACKET.DIGITAL_BYTES);
            pong = new TelemetryPacket((byte)PONGPACKET.IDENT, (int)PONGPACKET.ANALOG_BYTES, (int)PONGPACKET.DIGITAL_BYTES);
            buffer = new byte[512];
        ...
        public void Telemetry()
        {
            ...
            //if robot has been pinged, send pong, otherwise send normal telemetry
            byte[] req;
            if (pinged == true)
            {
                req = TransmitRequest(interfaceAddressBytes, frame, pong.GetBuffer());
                pinged = false; //resets pinged
            }
            else
            {
                req = TransmitRequest(interfaceAddressBytes, frame, telemetry.GetBuffer());
            }
            frame++;
            if (frame > 126) frame = 0;
            port.Write(req, 0, req.Length);
            ...
            if (port.BytesToRead > 0)
            {
                ...
                if (availableBytes > 0)
                {
                    ...
                    if (receiver.Fill(buffer, availableBytes))
                    {
                        ...
                        if (receiver.packet.data[0] == data.ident || receiver.packet.data[0] == ping.ident)
                        {
                            data.FillData(receiver.packet.data);

                            //if it was a pingpacket, set pinged to true, so the next outgoing packet will be a pong
                            if (receiver.packet.data[0] == ping.ident)
                            {
                                pinged = true;
                            }
        ...
        public byte[] TransmitRequest(byte[] addr, byte frameId, byte[] data)
        {
            ...
            //necessary packet stuff
            packet[0] = 0x7E;
            packet[1] = 0x00;
            packet[2] = (byte)(data.Length + 11);
            packet[3] = 0x00;
            packet[4] = frameId;
            //bytes 5-12 are used to store address
            for (i = 5; i < 13; i++)
            {
                packet[i] = addr[i - 5];
            }
            //byte 13 is spacer?
            packet[13] = 0x00;
            //bytes 14 through the end are used to store data
    ...
    public class InterfacePacket
    {
        ...
        public void FillData(byte[] bytes)
        {
            ...
            //starting at i++ means starting at analog.Length + 1
